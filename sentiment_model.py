...
from tensorflow_modules.modules import NeuralNetworkHiddenLayer,Convolution1DLayer,LSTMLayer,Convolution2DLayer
...
class SentimentCharCNNNeuralNetwork(Model):
    def __init__(self,FLAGS,word2id,char2id,word_embed_matrix,char_embed_matrix):
        self.FLAGS = FLAGS
        self.dataObject = SentimentDataObject(word2id, char2id, word_embed_matrix, char_embed_matrix,self.FLAGS.train_path, self.FLAGS.test_path, self.FLAGS.batch_size,self.FLAGS.review_length, self.FLAGS.word_length, self.FLAGS.discard_long,self.FLAGS.test_size)
        with tf.variable_scope('SentimentModel',initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, uniform=True)):
    ...
    def build_graph(self):
        channel_size_1=256
        filter1=[1,3,self.FLAGS.char_embedding_size,channel_size_1]
        strides_1=[1,1,1,1]
        pool_size_1=[1,1,2,1]
        layer1=Convolution2DLayer('ConvLayer1',filter1,strides_1,channel_size_1,pool_size_1)
        output1=layer1.build_graph(self.char_words)
        output1_shape=[1,ceil(ceil(self.FLAGS.review_length/strides_1[1])/pool_size_1[1]),ceil(ceil(self.FLAGS.word_length/strides_1[2])/pool_size_1[2]),channel_size_1]

        channel_size_2=256
        filter2=[1,3,channel_size_1,channel_size_2]
        strides_2=[1,1,1,1]
        pool_size_2= [1,1,3,1]
        layer2=Convolution2DLayer('ConvLayer2',filter2,strides_2,channel_size_2,pool_size_2)
        output2=layer2.build_graph(output1)
        output2_shape=[1,ceil(ceil(output1_shape[1]/strides_2[1])/pool_size_2[1]),ceil(ceil(output1_shape[2]/strides_2[2])/pool_size_2[2]),channel_size_2]

        channel_size_3=256
        filter3=[1,3,channel_size_2,channel_size_3]
        strides_3=[1,1,1,1]
        pool_size_3=[1,1,4,1]
        layer3=Convolution2DLayer('ConvLayer3',filter3,strides_3,channel_size_3, pool_size_3)
        output3=layer3.build_graph(output2)
        output3_shape=[1,ceil(ceil(output2_shape[1]/strides_3[1])/pool_size_3[1]),ceil(ceil(output2_shape[2]/strides_3[2])/pool_size_3[2]),channel_size_3]

        reshaped_output = tf.reshape(output3, [-1, output3_shape[1] * output3_shape[2] *output3_shape[3]])
        HIDDEN_LAYER1_SIZE = 4096
        HIDDEN_LAYER2_SIZE = 2048
        HIDDEN_LAYER3_SIZE = 1024
        layer4 = NeuralNetworkHiddenLayer('HiddenLayer1', output3_shape[1] * output3_shape[2] *output3_shape[3], HIDDEN_LAYER1_SIZE,
                                          self.keep_prob)
        ...
        with tf.variable_scope('full_layer1') as scope:
            full_weight1 = tf.get_variable(name='full_layer1_weight',
                                           shape=[HIDDEN_LAYER3_SIZE, self.FLAGS.ratings_size], dtype=tf.float32,
                                           initializer=tf.truncated_normal_initializer(stddev=0.05))
            full_bias1 = tf.get_variable(name='full_layer_bias', shape=[self.FLAGS.ratings_size], dtype=tf.float32,
    ...
    def add_loss(self):
        with vs.variable_scope('loss'):
            self.cost = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits, labels=self.ratings))
            self.correct_prediction = tf.equal(tf.argmax(self.final_output, 1), tf.argmax(self.ratings, 1),
